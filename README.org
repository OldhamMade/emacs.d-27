...
*** Use =mini-modeline=
    ...
    (use-package mini-modeline
      ...
      :custom
        ...
        '(:eval (string-trim-left (or (my/mode-line-buffer-name) ""))))
        (mini-modeline-r-format
        '(:eval (concat
            (my/mode-line-buffer-modified)
            (my/mode-line-readonly)
            (my/mode-line-line-column)
            (my/mode-line-input-method)
            (my/mode-line-line-encoding)
            (my/mode-line-buffer-encoding)
            (my/mode-line-indent-mode)
            (my/mode-line-mode-name)
            (my/mode-line-git-branch)
            my--mode-line-lsp
            my--mode-line-flymake
            (my/mode-line-flycheck)
            (my/mode-line-structural))))
    )
    #+end_src
*** Define custom =mini-modeline= functions
     #+begin_src emacs-lisp :tangle no
...
(defun my/mode-line-buffer-modified ()
  (when (and buffer-file-name (buffer-modified-p))
    (concat
"  "
     (propertize "[*]"
       'help-echo (concat (buffer-name) " has unsaved changes")))))
(defun my/mode-line-readonly ()
...
:custom
	(mini-modeline-face-attr `(:background ,(face-attribute 'default :background)))
	(mini-modeline-echo-duration 1)
  (setq mini-modeline-l-format (list
"%* "  ;; modified marker
"%b "  ;; buffer name
"[%m] "  ;; mode
"%l (%p)"  ;; line position
...
*** Make the modeline more minimal
    ...
    (use-package feebleline
      ...
      :hook
        (emacs-startup-hook . feebleline-mode))
    #+end_src
** Highlights
*** Indentation
...
*** Change cursor dynamically, depending on the context
    ...
    (use-package cursor-chg
      ...
      :hook (emacs-startup . change-cursor-mode))
    #+END_SRC
** Files
